/*
 * Here is a sample of how to use OptionsParser to
...
example( int argc, char* argv[] )
{
	...
	if( opt->getValue( "title" ) != NULL )
        	std::cout << "title = " << opt->getValue( "title" ) << endl ;
        if( opt->getFlag( 'c' ) )  
		std::cout << "c = flag set " << endl ;
        if( opt->getFlag( 'z' ) || opt->getFlag( "zip" ) )  
		std::cout << "zip = flag set " << endl ;
        std::cout << endl ;

	/* 7. GET THE ACTUAL ARGUMENTS AFTER THE OPTIONS */
	for( int i = 0 ; i < opt->getArgc() ; i++ ){
		std::cout << "arg = " <<  opt->getArgv( i ) << endl ;
	}
